...
( function() {
		...
		return {
			...
			data: function() {
				...
				this.parts.image.setAttributes( {
					...
					// This internal is required by the upload.
			...
			init: function() {
				...
				// Don't initialize resizer when upload.readOnly is set to true (#719).
	...
	function setupResizer( widget ) {
		...
		resizer.on( 'mousedown', function( evt ) {
			...
			// Attaches an event to a global document if inline upload.
			// Additionally, if classic (`iframe`-based) upload, also attaches the same event to `iframe`'s document.
	...
	// on upload configuration. Note that the following may not cover
 ...
 * must be supplied to the upload:
 *
 * * For {@glink guide/dev_framed classic upload} it can be done by defining additional
 * styles in the {@link CKEDITOR.config#contentsCss stylesheets loaded by the upload}. The same
 * styles must be provided on the target page where the content will be loaded.
 * * For {@glink guide/dev_inline inline upload} the styles can be defined directly
